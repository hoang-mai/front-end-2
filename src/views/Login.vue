<template>
  <div
    class="relative flex items-center justify-center min-h-screen bg-red-100"
  >
    <div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
      ...
      <form @submit.prevent="handleLogin" class="space-y-2">
        ...
        <div class="relative">
          ...
          <p v-else class="h-6"></p>
          <EyeOutlined @click="showPassword = true" v-if="!showPassword" class="text-xl absolute right-3 top-9 cursor-pointer"/>
          <EyeInvisibleOutlined @click="showPassword = false" v-else class="text-xl absolute right-3 top-9 cursor-pointer"/>
        </div>
        <button
          type="submit"
    ...
    >
      <svg class="w-full h-32" viewBox="0 0 500 150" preserveAspectRatio="none">
        <path
          d="M0.00,49.98 C149.99,150.00 349.44,-49.98 500.00,49.98 L500.00,150.00 L0.00,150.00 Z"
          style="stroke: none; fill: #fecaca"
        ></path>
        <!-- màu đỏ nhạt -->
      </svg>
    </div>
...
const handleLogin = () => {
  if (validateEmail() || validatePassword() || email.value === "" || password.value === "") {
