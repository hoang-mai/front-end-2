<template>
  <header class="relative bg-(--color-bg-red) text-(--color-text-white) p-4 flex justify-center items-center w-full">
    ...
    <div class="absolute right-4 flex items-center cursor-pointer gap-4">
      <a-tooltip placement="left" color="var(--color-text-red)" title="Thông báo">
        <a-dropdown :arrow="{ pointAtCenter: true }" :trigger="['click']" placement="bottomRight"
    ...
    <form ref="form" class="flex flex-col">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div>
          ...
          <p v-if="newPassword.length > 0 && newPassword.length < 6" class="text-red-500 text-sm">
  ...
  <a-modal v-if="selectedAnnouncement" v-model:open="openAnnouncement" :width="800" >
    ...
    <div class="flex flex-col gap-4">
      ...
      <p class="text-sm text-gray-500">
        Ngày tạo: {{ new Date(selectedAnnouncement.createdAt).toLocaleDateString("vi-VN") }}
      </p>
    </div>
    <template #footer>
      <button
