      ...
      <a-form :model="formState" name="createAccount" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }"
        ...
        <a-form-item label="Số điện thoại" name="phoneNumber">
          <a-input v-model:value="formState.phoneNumber" placeholder="Nhập số điện thoại" maxlength="10"
            @keypress="(e: KeyboardEvent) => { if (!/[0-9]/.test(e.key)) e.preventDefault(); }" :size="'large'" />
        </a-form-item>
        ...
        <a-form-item :wrapper-col="{ offset: 6, span: 18 }">
          <button html-type="submit"
            class="!bg-(--color-text-red) !text-(--color-text-white) !px-6 !py-2 !rounded-2xl !text-base !font-medium flex items-center gap-2">Tạo
            Tài Khoản</button>
        </a-form-item>
      </a-form>
    </a-card>
